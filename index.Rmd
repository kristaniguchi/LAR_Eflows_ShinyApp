...
sidebarPanel(
              radioButtons("location", label = "Select Location:",
                           choices = c("Node","Reporting Reach",
                                       "Master Plan LA River Reaches"),
                           selected="Node"),
              radioButtons("designateBU", label = "Designation:",
                           choices = dat$Designation_BU %>% unique() %>% .[-1],
                           selected= dat$Designation_BU %>% unique() %>% .[2]),
...
location_choice <- reactive({
  switch(input$location,
         ...
         "Node"= dat$Node,
         "Master Plan LA River Reaches" = dat$waterbody)
  })
```
...
if(input$location == "Reporting Reach"){
    ...
    selectInput("MasterLA",label = "Select Master Plan LA River Reaches:",
                choices = location_choice() %>% unique %>% sort)
  }
  ...
  selectInput("designations", label = "Beneficial Usage:",
...
selectInput("BU_names", label = "BU_Names:",
            choices = dat$BU_names  %>% unique() %>% strsplit(",") %>%
              unlist() %>% unique() %>% str_trim() %>% .[-1],
            selected = dat$BU_names  %>% unique() %>% strsplit(",") %>%
              unlist() %>% unique() %>% str_trim() %>% .[2]),
...
boxplot_dat <- reactive({
  ...
  if(input$location == "Node"){
    out <- dat %>% filter(Node == input$Node,
                          Designation_BU == input$designateBU |
                            is.na(Designation_BU),
                          Seasonal_Component %in% season,
                          Designation  == input$designations|is.na(Designation),
