...
Example sensivity curve and description of plots [flow- and species-based]
...
fluidPage(tags$h3("Location and Season"),
          fluidRow(column(4,
                   ...
                   column(4,
                          selectInput("sens_UBF", label = "Select UBF:",
                                      choices = c("UBF0","UBF50","UBF100")))))


sens_location_choice <- reactive({
  switch(input$sens_location,
         "Reporting Reach" = sensitivity_dat$Reporting_Reach,
         "Node"= sensitivity_dat$Node)
})
...
sens_curve <- reactive({
  if(input$sens_UBF == "UBF0"){
    validate(need(input$sens_season != ""," "))
    validate(need(input$node != ""," "))
    
    x = 0:100
    # UBF0
    F300p10Dry <- 0.2759 + 0.928049117261522*x
    F300p10Wet <- 0.12743000000000 + 0.860528401345517*x
    
    F319p10Dry <- 3.09190833333333 + 0.901283573271957*x
    F319p10Wet <- 3.23505083333333 + 0.858052131135869*x
    
    F57Cp10Dry <- 3.04752083333334 + 0.89780191138599*x
    F57Cp10Wet <-3.05811916666666 + 0.852257927992448*x
    
    LA13p10Dry <--0.0177541666666679 + 0.908978535421035*x
    LA13p10Wet <-0.0168766666666714 + 0.877785531664648*x
    
    LA11p10Dry<-1.42435833333334 + 0.907889677644299*x
    LA11p10Wet<-1.50892416666666 + 0.877071002008068*x
    
    F34Dp10Dry<-3.046120833 + 0.900674904*x
    F34Dp10Wet<-3.113899167 + 0.855259884*x
    

    
    LA14p10Dry<-0.170541667 + 0.917579994*x
    LA14p10Wet<-0.1094 + 0.8781502*x
  
    
    GLENp10Dry<-0.1044875 + 0.89905345*x
    GLENp10Wet<-0.064108333 + 0.876925846*x
    

    LA8p10Dry<-3.057025 + 0.896364706*x
    LA8p10Wet<-3.061306667 + 0.85220692*x
  
    
    LA3p10Dry<-3.075233333 + 0.901405241*x
    LA3p10Wet<-3.183428333 + 0.8571592*x
 

    F300p50Dry <- 0.471433333 + 0.98607692*x
    F300p50Wet <- 0.389243333 + 0.972782287*x
    
    
    F319p50Dry <- 3.141541667 + 0.984314288*x
    F319p50Wet <- 3.5314475 + 0.968478183*x
    
  
    F57Cp50Dry <-  3.120866667 + 0.982749468*x
    F57Cp50Wet <-3.502015 + 0.947470993*x
  
    
    LA13p50Dry <-0.200066667 + 0.972687005*x
    LA13p50Wet <-0.354046667 + 0.967654362*x
    

    LA11p50Dry<-1.500695833 + 0.9920234664*x
    LA11p50Wet<-1.847085 + 0.946765283*x
   

    F34Dp50Dry<-3.118075 + 0.983705477*x
    F34Dp50Wet<-3.546828333 + 0.955932799*x
    
    
    LA14p50Dry<-0.383633333 + 0.983570629*x
    LA14p50Wet<-0.443888333 + 0.968354829*x
   

    GLENp50Dry<-0.1272375 + 0.983284118*x
    GLENp50Wet<-0.3830925 + 0.947159945*x
    

    LA8p50Dry<-3.1217625 +0.98115916*x
    LA8p50Wet<-3.518124167 + 0.949074777*x
  
    
    LA3p50Dry<-3.145716667 + 0.98395374*x
    LA3p50Wet<-3.572595 + 0.964739136*x
    

    F300p90Dry <- 0.68965 + 1.037970368*x
    F300p90Wet <- 0.866066667 + 1.00608897*x
    
    
    F319p90Dry <- 3.3874875 + 1.09410519*x
    F319p90Wet <- 6.028019167 + 0.97194679*x
  

    
      
    F57Cp90Dry <- 3.4333625 + 1.079492919*x
    F57Cp90Wet <-3.948730833 + 0.977697646*x
    

    LA13p90Dry <-0.2375625 + 1.069487095*x
    LA13p90Wet <-0.621583333 + 1.007369839*x
    

    
    LA11p90Dry<-1.8197125 + 1.0893563778*x
    LA11p90Wet<-2.207846667 + 0.979230885*x
    

  
    F34Dp90Dry<-3.273470833 + 1.095089887*x
    F34Dp90Wet<-4.656798333 + 0.97639256*x
    

    LA14p90Dry<-0.543754167 + 1.070049625*x
    LA14p90Wet<-0.7274475 + 1.00785122*x


    GLENp90Dry<-0.487341667	 + 1.07843565*x
    GLENp90Wet<-0.68509 + 0.979851782*x
    
    
    LA8p90Dry<-3.267441667 + 1.093362021*x
    LA8p90Wet<-4.126330833 + 0.977142093*x
    

    LA3p90Dry<-3.374066667 + 1.096772517*x
    LA3p90Wet<-5.7195825 + 0.973521175*x
  ...
  sidebarLayout(
    ...
    sidebarPanel( selectInput("hab", "Beneficial Use:",
                  ...
                  selectInput("season2", "Season Baseflow:",
                              c("Wet" = "wet", "Dry" = "dry")),
                  selectInput("use", "Select Species:",textOutput("other"),
                              c( 
                                "Willow Adult",
...
heatmap_plot <- reactive({
  ...
  heatmap_colors <- setNames(c("#E7F0F1",hex[c(11,8,7)]), levels(dat3$value))
...
fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(width=3,selectInput("node_depth","Select Location:",
...
mainPanel(width=9,renderPlot({graphPlan()}))))
```
