...
FMM <- read.csv("raw/FFM_percentiles_wetdrybaseflowmag_02172021_baselineWRP.csv")
FlowRanges <- read.csv("raw/FlowRanges_Species_RecUses_Allnodes_04132021.csv")
baseline_dat <- reshape2::melt(read_csv("raw/summary_seasonalWRP_node_baselineonly.csv"))
```


```{r shape files}
load("data/shapefilesRdata.RData")
```

#  {.tabset .tabset-pills}

![](www/river_logo2.jpeg)
...
<font size="5"> Reference Map </font>


```{r}
LA_dat <- cbind.data.frame(river@coords,'Reporting_Reach'=river@data[,'Reporting_']) %>% 
  mutate(Reporting_Reach = c("Compton Creek","Compton Creek",
                             str_extract(Reporting_Reach[-c(1:2)],"^.*(?=( -))"))) %>% 
  distinct(Reporting_Reach,.keep_all = TRUE)


LA_map_reach <- leaflet(options = 
                          leafletOptions(minZoom = 8,maxZoom = 16, zoomControl = FALSE )) %>%
  htmlwidgets::onRender("function(el, x) {
        L.control.zoom({ position: 'topright' }).addTo(this)}") %>%
  setView(lng = -118.2437, lat =34.0422, zoom = 12)%>%
  addTiles(options = providerTileOptions(opacity = 0.3)) %>%
  addMarkers(data = LA_dat,lng = ~ coords.x1, lat = ~ coords.x2, 
             label = ~Reporting_Reach,labelOptions = labelOptions(noHide = T,textsize="13px")) %>% 
  addCircleMarkers(data=WRP_shp,lng= ~ Long, lat = ~ Lat,color = 'red', radius = 20,
                   label = WRP_shp$wrp,stroke = FALSE, fillOpacity = 0.4,
                   labelOptions = labelOptions(textsize="13px")) %>%
  addPolylines(data=river2@lines[[4]],weight=4,col='purple') %>% 
  addPolylines(data=river2@lines[[7]],weight=4,col='brown') %>% 
  addPolylines(data=river2@lines[[10]],weight=4,col='green') %>% 
  addPolylines(data=river2@lines[[15]],weight=4,col='#5490d6') %>% 
  addPolylines(data=river2@lines[[16]],weight=4,col='#5490d6') %>% 
  addPolylines(data=river2@lines[[17]],weight=4,col='blue') %>% 
  addPolylines(data=river2@lines[[18]],weight=4,col='purple') %>% 
  addPolylines(data=river2@lines[[19]],weight=4,col='#ff7003') %>% 
  addPolylines(data=river2@lines[[20]],weight=4,col='brown') %>% 
  addPolylines(data=river2@lines[[21]],weight=4,col='#b55d19') %>% 
  addPolylines(data=river2@lines[[22]],weight=4,col='#9041ab') %>% 
  addPolylines(data=river2@lines[[23]],weight=4,col='green') %>%
  addPolylines(data=river2@lines[[24]],weight=4,col='red') %>% 
  addPolylines(data=ws_map@polygons[[1]],weight=3,col='black') %>% 
  addPolylines(data = reporting_reaches@lines[[1]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[2]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[3]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[5]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[6]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[8]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[9]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[11]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[12]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[13]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[14]],weight=3,color = "gray") %>% 
  addPolylines(data = reporting_reaches@lines[[23]],weight=3,color = "gray") 
...
```{r}
LA_map <- reactive({leaflet(options = leafletOptions( 
  minZoom = 8,     maxZoom = 16, zoomControl = FALSE )) %>%
    ...
    addMarkers(lng=-118.247706, lat=34.108175,
               popup=PopcontentLA11,
               label="LA11",
               labelOptions = labelOptions(noHide = T,textsize="12px"))%>% #MarkerHome
    addMarkers(lng=-118.514642, lat=34.184708,popup=PopcontentLA20,
               label="LA20",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.484394, lat=34.173175,
               popup=PopcontentLA20_2,
               label="LA20_2",
               labelOptions = labelOptions(noHide = T,textsize="12px"))%>%
    addMarkers(lng=-118.173592, lat=33.948297,
               popup=PopcontentF34D,
               label="F34D",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.228233, lat=34.085803,
               popup=PopcontentF57C,
               label="F57C",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.275339, lat=34.136858,
               popup=PopcontentGLEN,
               label="GLEN",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.196333, lat=33.8639,popup=PopcontentLA3,
               label="LA3",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.228603, lat=34.040528,
               popup=PopcontentLA8,
               label="LA8",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.299758, lat=34.156839,popup=PopcontentLA14,
               label="LA14",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.205728, lat=33.817733,popup=PopcontentF319,
               label="F319",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.378453, lat=34.1411,popup=PopcontentF300,
               label="F300",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.278844, lat=34.150964,popup=PopcontentLA13,
               label="LA13",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.211672, lat=33.851269,
               label="F37B_Low",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>% 
    addMarkers(lng=-118.222186, lat=33.878144,
               label="F37B_High",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addMarkers(lng=-118.072069, lat=34.049108,
               label="11101250",
               labelOptions = labelOptions(noHide = T,textsize="12px")) %>%
    addPolylines(data=river_mainstem,weight=4,col = 'blue') %>% 
    addPolylines(data=ws_map@polygons[[1]],weight=3,col='black') %>% 
    addPolylines(data = reporting_reaches@lines[[4]],weight=3,color = 'blue') %>% 
    addPolylines(data = reporting_reaches@lines[[10]],weight=3,color = 'blue') %>% 
    addPolylines(data = reporting_reaches@lines[[24]],weight=3,color = 'blue') 
...
Flow ranges associated with designated existing and potential uses and possible future beneficial uses were developed based on needs of the associated focal species (and life stages) and recreational uses for each relevant location (or node) along the river (Table 1). Relationships between focal species and beneficial uses were developed in consultation with the project's Technical Advisory Committee. Although cold freshwater habitat (COLD) is not currently a designated beneficial use in the mainstem of the LA River, one goal of this study is to evaluate whether proposed management actions could influence flow conditions that have the potential to support non-designated beneficial uses, such as COLD, in the future

*Table 1. Focal species associated with beneficial uses.  Highlighted species do not currently occur in the mainstem of the LA River.  Cold freshwater habitat (COLD) is currently not a designated beneficial use for any LA River reach covered by this study*

![](www/table1.PNG)

*<font size="1">1WARM: Warm freshwater habitat</font>*<br />
*<font size="1">2EST: Estuarine habitat</font>*<br />
*<font size="1">3WILD: Wildlife habitat</font>*<br />
*<font size="1">4RARE: Rare, threatened, or endangered species</font>*<br />
*<font size="1">5MIGR: Migration of aquatic organisms</font>*<br />
*<font size="1">6SPWN: Spawning, reproduction, and/or early development</font>*<br />
*<font size="1">7COLD: Cold freshwater habitat</font>*


Given the extensive set of flow ranges for multiple species and locations, a process was developed to synthesize flow ranges into more integrative flow management targets based on a series of decisions (Figure 1).  Click on the Flow Range Determination tab at the top to explore flow management targets based on user-defined selections.



![](www/FigureX_flowrecommendationprocess.jpg)

*Figure 1. Proposed process for synthesizing and interpreting flow ranges to develop overall flow management target. Medium probability is defined as 50% of the maximum probability of occurrence; high probability is defined as 75% of the maximum probability of occurrence.  Low probability was determined to not be a desirable management goal*
...
Potential changes to wastewater discharge and dry weather stormdrain discharge were simulated using a “sensitivity curve” approach that relates discharge to instream flow conditions as measured by functional flow metrics. Instead of identifying a finite set of scenarios, sensitivity curves allow for consideration of a broad range of management options based on the amount of discharge to the river. Wastewater discharge scenarios were based on a Monte Carlo simulation  which evaluated the effects of 500 randomly selected scenarios ranging from 0-100% of current discharge, representing multiple combinations of potential WRP discharge reductions from each plant. 


A suite of flow-based sensitivity curves was developed for 13 reporting nodes along the river, two seasonal functional flow components (wet-season baseflow and dry-season baseflow), and for multiple management scenarios (WRP discharge and dry weather stormdrain discharge scenarios). Sensitivity curves were only developed for the wet- and dry-season baseflow magnitude metrics because they were the most sensitive flow metrics to changes in WRP discharge and dry weather stormdrain reductions.  A process was developed to use the flow ranges associated with different scenarios with the sensitivity curves to evaluate how much and under which scenarios flows can be reduced and still meet flow ranges that support a majority of beneficial uses for a given location along the river 


![](www/FigureX_SensitivityCurveProcess.jpg)

*Figure 2. Proposed process to use optimal flow range of specific management targets with sensitivity curves to evaluate WRP and dry weather stormdrain scenarios. The optimal flow range can be derived from the process shown in Figure 3*


An example application of sensitivity curves at Glendale Narrows (GLEN), shows the optimal flow range for Willow and Typha plotted with the flow-based sensitivity curves representing changes in both WRP and dry weather stormdrain discharge (Figure 3).

![](www/figure3.png)

*Figure 3. Flow-based sensitivity curves illustrating the combined effects of change in WRP discharge and 50% (yellow) and 100% reduction (red) in dry weather stormdrain discharge at GLEN reporting node. Solid and dashed lines represent median values; bounds of the gray and colored bands are the 10th and 90th percentile dry-season instream flows across the modeled period.  Optimal flow range in this example is based on a 50% (medium) probability of occurrence of Willow and Typha*


To explore the flow-based and species-based sensitivity curves by location, season, and species, click on the Sensitivity Curves tab at the top of this page

------------------------------------------------------------------------

<font size="5">Flow Range Heat Maps</font>

Reductions in WRP discharge could have cumulative impacts to various beneficial uses along the LA River. A heat map was developed to evaluate spatially how various WRP discharges may impact instream flow conditions needed to support various species and recreational uses. To explore the heat map under various WRP scenarios, click on the Flow Range Heat Map tab at the top of this page

------------------------------------------------------------------------

<font size="5">Flow Depth Visualizer</font>

Changes in discharge will affect the hydraulic conditions (i.e., depth and wetted width) of a stream in different ways depending on the site-specific channel morphology. The Flow Depth Visualizer tab allows the user to visualize the flow depth and wetted width associated with various discharges for all reporting node locations.

## Flow Range Determination {.tabset .tabset-pills}
...
```
<p>Species notes: SAS is Santa Ana Sucker;  Cladophora is green algae; Typha is cattail</p>
Flow metric notes: wet-season baseflow is the magnitude of wet season baseflow (10th percentile of daily flows, during the wet season, including peak flow events); dry-season baseflow is the dry season baseflow magnitude (the 50th percentile of daily flow, during the dry season); wet season peak flow is the 99th percentile of daily flow for the water year.
...
fluidPage(tags$h3("Species"),
          fluidRow(column(3,selectInput("sens_species",label="Select Species:",
                   ...
                   column(3,
                          selectInput("sens_UBF", label = "Select Stormdrain Reduction:",
                                      choices = c("None","50% Reduction","100% Reduction",
...
filtered_baseline <- reactive({
  ...
  out <- baseline_dat %>% filter(
    ReportingNode == input$node,
    variable == input$sens_season)
...
sens_curve <- reactive({
  ...
  base_plot <- ggplot()+
    ...
    geom_ribbon(data =plotter_UBF100,
                ...
                fill = '#c3d0e6', alpha = .5)+
    geom_vline(xintercept = filtered_baseline()$value,linetype='dashed')+
    labs(x = paste0("Average Annual ",
                    str_to_title(input$sens_season),"-Season WRP Discharge (cfs)"), 
  ...
  if(input$sens_UBF != "None"){
    ...
    if(input$sens_UBF =="50% Reduction"){
      g <- base_plot +
    ...
    else if(input$sens_UBF == "100% Reduction"){
...
```
<p>Species notes: Typha is cattail </p>
Flow metric notes: wet-season baseflow is the magnitude of wet season baseflow (10th percentile of daily flows, during the wet season, including peak flow events); dry-season baseflow is the dry season baseflow magnitude (the 50th percentile of daily flow, during the dry season)
```{r}
...
```{r}
limiting_factors <- read_csv("raw/limitingfactors.csv") %>% 
  mutate(Season = case_when(Season=="Dry" ~ "dry",
                            Season=="Wet"~ "wet")) %>% 
  filter(!species.lifestage.hyd %in% c("Cladophora Adult Depth","Cladophora Adult Velocity"))
```
...
column(12,
       column(4,
              renderUI({
                tmpdf <- limiting_factors %>%
                  filter(ReportingNode == input$node) %>% 
                  filter(Season == input$sens_season)
                selectInput("species.lifestage", 
                            "Select a Species Lifestage",
                            sort(unique(tmpdf$species.lifestage)) )
              })),
       column(4,tags$h3(""),
...
SpeciesProb_dat <- reactive({
  validate(need(input$node != ""," "))
  validate(need(input$sens_season != ""," "))
  validate(need(input$species.lifestage != ""," "))
  
  if(input$sens_season == "dry"){
    out <- SpeciesProb %>%
      filter(ReportingNode == input$node) %>% 
      filter(season == input$sens_season) %>% 
      filter(species.lifestage == input$species.lifestage) %>%
      rename(seasonal.wrp.Q = dry_season)
  }else{
    out <- SpeciesProb %>%
      filter(ReportingNode == input$node) %>% 
      filter(season == input$sens_season) %>% 
      filter(species.lifestage.hyd == input$species.lifestage) %>%
...
SpeciesProb_plot <- reactive({
  ...
  baseline <- dat %>% filter(Scenario==0) %>% pull(seasonal.wrp.Q)
  species_lifestage <- dat$species.lifestage.hyd
  req(dat$seasonal.wrp.Q)
...
fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel( selectInput("hab", "Beneficial Use:",
                              c("ALL", "WILD","RARE", "MIGR", "SPWN", "COLD")),
...
```
<p>Species notes: SAS is Santa Ana Sucker;  Cladophora is green algae; Typha is cattail </p>
Flow metric notes: wet-season baseflow is the magnitude of wet season baseflow (10th percentile of daily flows, during the wet season, including peak flow events); dry-season baseflow is the dry season baseflow magnitude (the 50th percentile of daily flow, during the dry season). Not applicable means that the focal species or recreational uses are not currently observed and will likely not be supported at the node.  Within range means that the current flows support the beneficial use. 
        
```{r}
checkboxInput("showlabel", "Show Labels", FALSE)
```
...
la_map <- reactive({
  ...
  LosAngeles_nomarkers <- leaflet(options = leafletOptions( 
    minZoom = 8,     maxZoom = 16, zoomControl = FALSE )) %>%
    htmlwidgets::onRender("function(el, x) {
        L.control.zoom({ position: 'topright' }).addTo(this)}") %>%
...
```{r}
LA20_2 <- read.csv("data/XC/XC_LA20_2.csv")
colnames(LA20_2) <- c("Station", "Elevation")

LA11<- read.csv("data/XC/XC_LA11.csv")
colnames(LA11) <- c("Station", "Elevation")


LA11101<- read.csv("data/XC/XC_11101250.csv")
colnames(LA11101) <- c("Station", "Elevation") 


LAF34D<- read.csv("data/XC/XC_F34D.csv")
colnames(LAF34D) <- c("Station", "Elevation")


LAF37B_High<- read.csv("data/XC/XC_F37B_High.csv")
colnames(LAF37B_High) <- c("Station", "Elevation")


LAF3_Low<- read.csv("data/XC/XC_F37B_Low.csv")
colnames(LAF3_Low) <- c("Station", "Elevation")


LAF45B<- read.csv("data/XC/XC_F45B.csv")
colnames(LAF45B) <- c("Station", "Elevation")


LAF57C<- read.csv("data/XC/XC_F57C.csv")
colnames(LAF57C) <- c("Station", "Elevation")


LAF300<- read.csv("data/XC/XC_F300.csv")
colnames(LAF300) <- c("Station", "Elevation")


LAF319<- read.csv("data/XC/XC_F319.csv")
colnames(LAF319) <- c("Station", "Elevation")


GLEN<- read.csv("data/XC/XC_GLEN.csv")
colnames(GLEN) <- c("Station", "Elevation")


LA1<- read.csv("data/XC/XC_LA1.csv")
colnames(LA1) <- c("Station", "Elevation")


LA2<- read.csv("data/XC/XC_LA2.csv")
colnames(LA2) <- c("Station", "Elevation")


LA3<- read.csv("data/XC/XC_LA3.csv")
colnames(LA3) <- c("Station", "Elevation")


LA8<- read.csv("data/XC/XC_LA8.csv")
colnames(LA8) <- c("Station", "Elevation")


LA13<- read.csv("data/XC/XC_LA13.csv")
colnames(LA13) <- c("Station", "Elevation")


LA14<- read.csv("data/XC/XC_LA14.csv")
colnames(LA14) <- c("Station", "Elevation")


LA20<- read.csv("data/XC/XC_LA20.csv")
...
graphPlan <- reactive({
  ...
  if(input$node_depth == "LA20_2"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA20_2 - Los Angeles River above Sepulveda Basin"
  }
  if(input$node_depth == "LA11"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA11 - Los Angeles River at Glendale Narrows"
  }
  if(input$node_depth == "F34D"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F34D - Los Angeles River above confluence with Rio Hondo"
  }
  if(input$node_depth == "F37B_High"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F37B_High - Upper Compton Creek"
  }
  if(input$node_depth == "F37B_Low"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F37B_Low - Lower Compton Creek"
  }
  if(input$node_depth == "F45B"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F45B - Rio Hondo below spreading grounds"
  }
  if(input$node_depth == "F57C"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F57C - Los Angeles River above confluence with Arroyo Seco"
  }
  if(input$node_depth == "F300"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F300 - Los Angeles River above confluence with Burbank Channel"
  }
  if(input$node_depth == "F319"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "F319 - Los Angeles River at Wardlow Gage"
  }
  if(input$node_depth == "GLEN"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "GLEN - Los Angeles River below Glendale WRP"
  }
  if(input$node_depth == "LA3"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA3 - Los Angeles River below confluence with Rio Hondo"
  }
  if(input$node_depth == "LA8"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA8 - Los Angeles River above confluence with Rio Hondo"
  }
  if(input$node_depth == "LA13"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA13 - Los Angeles River above Glendale WRP"
  }
  if(input$node_depth == "LA14"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA14 - Los Angeles River below confluence with Burbank Channel"
  }
  if(input$node_depth == "LA20"){
    ...
    n <- length(y)
    x <- fix$Station 
    name <- "LA20 - Los Angeles River within Sepulveda Basin"
  }
  ...
  plot(x, y, type='n', xlab="Station (ft)", ylab="Depth (ft)", 
       main = name, cex.main = 2, col.main = '#2E8B57',
       cex.lab = 1.5, cex.sub = 3, col.lab = "#228B22", font.lab = 3, font.main = 3)
...
```
Note: cross-sections are approximate and were smoothed using a spline function. 
```{r}
renderLeaflet({LA_map() })
```
