...
river_reporting <- readOGR("data/reporting_reaches/LAR_mainstem.shp")
river_reporting <- spTransform(river_reporting, CRS("+proj=longlat +datum=WGS84 +no_defs"))
...
```{r}
LA_map2 <- reactive({leaflet(options = leafletOptions( 
  minZoom = 8,     maxZoom = 16, zoomControl = FALSE )) %>%
  htmlwidgets::onRender("function(el, x) {
        L.control.zoom({ position: 'topright' }).addTo(this)}") %>%
  setView(lng = -118.2437, lat =34.0522, zoom = 11)%>%
  ...
  addMarkers(lng=-118.278844, lat=34.150964,popup=PopcontentLA13,
             label="LA13",
             labelOptions = labelOptions(noHide = T,textsize="13px")) %>%
  addPolylines(data=river_reporting,weight=4,col = 'blue')
})
```
