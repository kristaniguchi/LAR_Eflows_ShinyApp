...
![](www/LAR_watershedMap_node_reaches.png)
       ...
       column(4,
              renderUI({
                ...
                selectInput("species.lifestage", 
                            ...
                            sort(unique(tmpdf$species.lifestage.hyd)) )
              })),
       column(4,tags$h3(""),
...
column(4,
              renderUI({
                req(input$species.lifestage)
                tmpdf <- SpeciesProb %>%
                  filter(ReportingNode == input$node) %>% 
                  filter(season == input$sens_season) %>% 
                  filter(species.lifestage == input$species.lifestage)
                selectInput("species.lifestage.hyd", 
                            "Depth/Velocity",
                            sort(unique(tmpdf$species.lifestage.hyd))) })        
       )
```
...
SpeciesProb_dat <- reactive({
  ...
  #validate(need(input$species.lifestage.hyd != ""," "))
  ...
  if(input$sens_season == "dry"){
    out <- SpeciesProb %>%
      ...
      #filter(species.lifestage.hyd == input$species.lifestage.hyd) %>% 
