...
synthesis_current_flows <- reactive({
  ...
  dat <- boxplot_dat() %>% filter(Species=="Current Flow") %>%
...
synthesis_table <- reactive({
  out <- tibble("Season" = "Summer Baseflow",
                "Species" = c("Willow","Typha"),
                "current-flow ranges" = c(ranges_print(round(
                  synthesis_current_flows()[[1]])),
                ranges_print(round(
                  synthesis_current_flows()[[1]]))),
                "Magnitude" = c(Reduce(intersect,
