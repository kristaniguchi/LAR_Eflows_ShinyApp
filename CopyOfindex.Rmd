...
SpeciesProb <- read_csv("raw/SynthesisCurves_SpeciesProb.csv") %>%
...
```{r}
fluidPage(tags$h3("Species Sensitivity Curves"),
          fluidRow(
            column(4, 
                   renderUI({
                     tempdf <- SpeciesProb %>% filter(ReportingNode == input$node)
                     tempdf_species <- unique(tempdf %>% pull(species.lifestage))
                     selectInput("sens_species2",label="Select Species:",
                                 choices = tempdf_species )
                     })
                   )
          )
)
...
SpeciesProb_dat <- reactive({
  validate(need(input$node != ""," "))
  validate(need(input$sens_season != ""," "))
  validate(need(input$sens_species2 != ""," "))
  
  out <- SpeciesProb %>%
    filter(ReportingNode == input$node) %>% 
    filter(season == input$sens_season) %>% 
    filter(species.lifestage == input$sens_species2)
