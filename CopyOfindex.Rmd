...
BUnames <- reactive({
  if(input$location=="Node"){
    ...
    out <- c("All",dat %>% filter(Node == input$Node) %>%
               pull(BU_names) %>% strsplit(",") %>% unlist() %>%
               unique() %>% str_trim() %>% .[-1])
    return(out)    
  }
  else if(input$location == "Reporting Reach"){
    validate(need(input$Reach != ""," "))
    out <- c("All",dat %>% filter(Reach == input$Reach) %>%
               pull(BU_names) %>% strsplit(",") %>% unlist() %>%
               unique() %>% str_trim() %>% .[-1])
    return(out)      
  }
    ...
    out <- c("All",dat %>% filter(waterbody == input$MasterLA) %>%
               pull(BU_names) %>% strsplit(",") %>% unlist() %>%
               unique() %>% str_trim() %>% .[-1])
    return(out)       
  }
...
synthesis <- reactive({
  ...
  if(input$season != "All"){
    ...
    return(list(out,seasonal_ranges))
    }
  else{
...
select_species <- reactive({
  out <- boxplot_dat() %>% filter(!Species %in% c("Rec. Use - Kayak",
                                                  "Rec. Use - Fishing",
                                                  "Current Flow")) %>%
...
synthesis_table <- reactive({
  
  out <- bind_cols(synthesis()[[1]],
...
renderUI({
selectInput("species_selection", label = "Select Intersection of Species:",
            choices = synthesis_table() %>% pull(Species) %>% strsplit(" - ") %>%
              unlist() %>%
              unique() %>%
...
renderPrint({paste0(input$species_selection," - ","Adult")})

renderPrint({   Reduce(intersect,
                      synthesis()[[2]][['Summer Baseflow']]
                       [grep(paste0(input$species_selection," - ","Adult"),
                             synthesis_table() %>% pull(Species))] 
                      %>% ranges_print)
...
DT::renderDataTable({synthesis_table()},options = list(dom = 't'))
```
