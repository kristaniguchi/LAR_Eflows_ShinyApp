...
fluidPage(tags$h3("Location and Season"),
          fluidRow(column(4,
                          radioButtons("sens_location", 
                                       label = "Select Location:",
                                       choices = c("Node","Reporting Reach"),
                                       selected="Node")),
                   column(4,renderUI({
                     if(input$sens_location == "Reporting Reach"){
                       selectInput("sens_Reach",label = "Specific Location:",
                                   choices = sens_location_choice() %>%
                                     unique() %>% sort)
                     }
                     else if (input$sens_location == "Node"){
                       #selectInput("sens_Node",label = "Specific Location:",
                       #           choices = sens_location_choice() %>%
                       #             unique)
...
```{r, eval=FALSE}
sensitivity_curves_dat <- reactive({
  if(input$sens_location == "Node"){
    validate(need(input$sens_Node != ""," "))
    
    out <- sensitivity_dat %>% filter(Node == input$sens_Node,
                                      season == input$sens_season)
    return(out)
  }
  else{
    validate(need(input$sens_Reach != ""," "))
    out <- sensitivity_dat %>% filter(Reporting_Reach == input$sens_Reach,
                                      season == input$sens_season)
    return(out)
  }
})
...
hab_choice <- reactive({
  switch(input$hab,
        ...
        "Kayak",
        "Fishing"),
        "COLD" = c("SAS Adult","Steelhead Migration (Prolonged)",
             "Steelhead Migration (Burst)"),
        "SPWN" = "SAS Adult",
        'MIGR' = c("Steelhead Migration (Prolonged)",
             "Steelhead Migration (Burst)"),
        "Recreation" = c("Kayak","Fishing"),
        "RARE" = c("SAS Adult",
            "Steelhead Migration (Burst)",
            "Willow Adult",
            "Steelhead Migration (Prolonged)"),
        "WILD" = c("Willow Adult", "Typha Adult",
           "Cladophora Adult"),
        "EST" = c("Steelhead Migration (Prolonged)",
             "Steelhead Migration (Burst)",
             "Cladophora Adult"))
})
```
...
fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
                           renderPlot({heatmap_plot()},height=600,width=950)
                  )
...
heatmap_plot <- reactive({
  ...
  ggplot(dat3, aes(variable, Node)) + geom_tile(aes(fill = value),
                                                colour = "white") + 
    scale_fill_manual(values=hex[c(11,8,7,6)])+
    theme_bw()+
