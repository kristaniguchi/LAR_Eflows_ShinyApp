...
BUnames <- reactive({
  if(input$location=="Node"){
    ...
    out <- c("All", dat %>% filter(Node == input$Node) %>% pull(BU_names) %>%
               str_extract_all('[A-Z]+') %>% unlist() %>% unique() %>% na.omit)
    return(out)    
  }
  else if(input$location == "Reporting Reach"){
    validate(need(input$Reach != ""," "))
    out <- c("All", dat %>% filter(Reach == input$Reach) %>% pull(BU_names) %>%
               str_extract_all('[A-Z]+') %>% unlist() %>% unique() %>% na.omit)
    return(out)      
  }
  else{
    validate(need(input$MasterLA != ""," "))
    out <- c("All", dat %>% filter(waterbody == input$MasterLA) %>% pull(BU_names) %>%
               str_extract_all('[A-Z]+') %>% unlist() %>% unique() %>% na.omit)
    return(out)       
  }
  ...
  if(input$location=="Node"){
    dat <- dat %>% 
      ...
      mutate(flow_range = str_c(floor(Lower_Limit), ceiling(Upper_Limit), sep = '-'),
         Species_Label = fct_drop(Species_Label))
    dat2 <- dat %>% 
    ...
    if(nrow(dat2) > 0) {
      ADDBOX <- TRUE
      dat2 <- dat2 %>% 
        mutate(Lower_Limit = max(Lower_Limit, na.rm = T), Upper_Limit = min(Upper_Limit, na.rm = T)) %>% 
        ...
        mutate(label_range = glue("{species}:\n{range} cfs", species = species, 
                                        range = str_c(Lower_Limit, Upper_Limit, sep = '-')),
